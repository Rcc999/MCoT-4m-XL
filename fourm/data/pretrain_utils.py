...
def get_train_dataloader(dataset_config, modality_info, sampling_weights, text_tokenizer, input_size, 
                         ...
                         num_tasks, num_workers, dataset_batch_size=None, epoch_size=None, use_text_to_caption_transform=True):
    ...
    if dataset_config['type'] == 'multimodal':
        ...
        if image_domain_key is None:
            # Fallback or raise error if no rgb domain found
            # For simplicity, let's assume RandomCropImageAugmenter's default 'rgb' might work or is intended if specific key not found
        ...
        loader = build_huggingface_pretraining_dataloader(
            ...
            input_tokens_range=(min_input_tokens, min_input_tokens),
            target_tokens_range=(min_target_tokens, min_target_tokens),
            num_gpus=num_tasks, num_workers=num_workers,
            batch_size=dataset_batch_size, epoch_size=epoch_size,
...
def get_val_dataloader(dataset_config, dataset_name, train_configs, modality_info, sampling_weights, text_tokenizer, 
                       ...
                       dist_eval, num_tasks, num_workers, batch_size, pin_mem, use_text_to_caption_transform=True):
