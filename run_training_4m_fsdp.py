...
def setup_data(args):
    ...
    # Extract text_to_caption setting from extras section (default to True for backward compatibility)
    actual_use_text_to_caption_transform = data_config.get('extras', {}).get('text_to_caption', True)
    ...
    num_workers = args.num_workers # Default num_workers

    if num_trainsets == 1:
        # For single dataset, bypass MixtureDataset and configure the loader directly
        ...
        # Directly configure the single data loader with args.batch_size and args.epoch_size
        data_loader_train = get_train_dataloader(
            ...
            num_tasks=args.num_tasks, num_workers=num_workers, # Use args.num_workers
            dataset_batch_size=args.batch_size, # Pass main batch_size
            epoch_size=args.epoch_size,       # Pass main epoch_size
            use_text_to_caption_transform=actual_use_text_to_caption_transform
        )
        # Adjust num_workers if the dataset has a limited number of shards
        ...
        # Use MixtureDataset approach for multiple datasets
        train_iters = []
        shards_per_dataset = []
        ...
        # Adjust num_workers for the MixtureLoader based on the minimum shards available across datasets
    ...
    # This calculation should be robust for both cases
